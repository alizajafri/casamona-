...
function listing_sales(){
	...
	echo '<div class="flickity">
	  <div class="gallery js-flickity" data-flickity="{ "cellAlign": "left", "contain": true, "wrapAround": true }">';
...
<div class="listings-panel clearfix">
	...
	while ( $the_query->have_posts() ) : $the_query->the_post(); ?>
	  ...
	  <?php $field_key_gallary = get_field_object('field_58d366980a2bd', get_the_ID()); ?>
		<div class="post-thumb col-md-4 col-sm-4 col-xs-12 pull-left"><a href="<?php echo get_the_permalink(); ?>">
		<?php 
		...
		<div class="slide-content col-md-8 col-sm-8 col-xs-12 pull-right">
			<div class="col-md-8 col-sm-8 col-xs-12 pull-left"><div class="post-title"><h5><?php echo wp_trim_words( get_the_title(), 8, '...' ); ?></h5></div>
...
function new_rent_apartment(){
	...
	$the_query = new WP_Query( $args );
	//echo '<pre>';print_r($the_query);echo '</pre>';
	echo '<div class="row new_apartment"><div class="col-md-12 col-sm-12 col-xs-12 nw_rent_aprtmnt">';
	if ( $the_query->have_posts() ) :
	while ( $the_query->have_posts() ) : $the_query->the_post(); ?>
	  <div class="col-md-4 col-sm-4 col-xs-12 nv-rent-aprtmnt">
	    <div class="nv-rentl">
		<div class="post-thumb">
			<a href="<?php echo get_the_permalink(); ?>">
			<?php the_post_thumbnail(array(283,221)); ?>
			</a>
		</div>
		<div class="slide-content">
			...
			foreach ( $terms as $term ) 
			{
				if( 0 != $term->parent )
				{
					echo $term->name.'<span class="post-term">,</span>';
				}
			} ?>
			</div>
			<div class="post-meta">
			 <span class="price">&euro; <?php echo get_post_meta($post_id,'_price',true); ?></span>
			 <span class="listing_id pull-right">REF: <?php echo get_post_meta($post_id,'_listing_id',true); ?></span>
			 </div>
			<div class="post-meta-data pull-left">
			<span class="homes pull-left icon">
			<span class="icon-home4"></span>
			<?php echo get_post_meta($post_id,'_details_4',true); ?> m2</span>
			<span class="bed pull-left icon">
			<span class="icon-bed"></span>
			<?php if(get_post_meta($post_id,'_details_1',true)!='')
			 {
				echo get_post_meta($post_id,'_details_1',true);
			 } else
			 {
				echo '0';
			 } ?> beds</span>
			 <span class="bath pull-left icon">
			 <span class="icon-bathtub"></span>
			 <?php if(get_post_meta($post_id,'_details_2',true)!='')
			  {
				  echo get_post_meta($post_id,'_details_2',true);
			  } else
				  {
					  echo '0';
				  } ?> baths</span>
...
add_shortcode('new_rent_apartment','new_rent_apartment');
?>